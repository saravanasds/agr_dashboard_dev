...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                console.log(token);
                const response = await axios.get('http://3.25.59.94:9000/api/admin/withdrawRequestUser', {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
    ...
    const handleAddTodo = async () => {
        if (inputText.trim() !== "") {
            ...
            const token = localStorage.getItem('token');
            try {
                const response = await fetch('http://3.25.59.94:9000/api/admin/notification', {
                    method: 'POST',
                    headers: {
